...
export const useLiveSession = ({ systemInstruction, voiceName, onTurnComplete }: UseLiveSessionProps) => {
  ...
  // FIX: Added to hold the session promise to avoid race conditions.
  ...
  const cleanupSession = useCallback(() => {
      ...
      // FIX: Clear the session promise ref on cleanup.
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      // FIX: Store the session promise in a ref.
      sessionPromiseRef.current = sessionPromise;
      sessionRef.current = await sessionPromise;
      
      // Initialize audio contexts after session is confirmed
  ...
  const startAudioInput = useCallback(async () => {
      ...
      try {
        ...
        scriptProcessor.onaudioprocess = (audioProcessingEvent) => {
            ...
            // FIX: Use the session promise to send data to avoid race conditions and stale closures.
